...
def run(data,
    ...
    # eval
    model.eval()
    pred_result, targets, vis_outputs, vis_imgs = val.predict(model, dataloader, task)
    eval_result = val.eval(pred_result, targets, model, task)
    return eval_result, vis_outputs, vis_imgs


def main(args):
    eval_result, vis_outputs, vis_imgs = run(**vars(args))
    for i in range(10):
        LOGGER.info (('iou: ' + '%.2f:%.2f' + ' | AP: %.4f | Recall: %.4f') % ((0.5 + i * 0.05), (0.5 + (i + 1) * 0.05), eval_result[5][i], eval_result[6][i]))
    LOGGER.info(("AP: %.4f | AP@0.5: %.4f | AP@0.75: %.4f | AP@0.50:0.95: %.4f | recall: %.4f") % (eval_result[0], eval_result[1], eval_result[2], eval_result[3], eval_result[4]))
