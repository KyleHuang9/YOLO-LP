...
def write_tblog(tblogger, epoch, results, losses):
    ...
    tblogger.add_scalar("val/AP", results[0], epoch + 1)
    tblogger.add_scalar("val/mAP@0.5", results[1], epoch + 1)
    tblogger.add_scalar("val/mAP@0.50:0.95", results[2], epoch + 1)
    tblogger.add_scalar("val/recall", results[3], epoch + 1)

    tblogger.add_scalar("train/iou_loss", losses[0], epoch + 1)
    tblogger.add_scalar("train/corner_loss", losses[1], epoch + 1)
    tblogger.add_scalar("train/dist_focalloss", losses[2], epoch + 1)
    tblogger.add_scalar("train/cls_loss", losses[3], epoch + 1)
    tblogger.add_scalar("train/pro_loss", losses[4], epoch + 1)
    tblogger.add_scalar("train/alp_loss", losses[5], epoch + 1)
    tblogger.add_scalar("train/ads_loss", losses[6], epoch + 1)
