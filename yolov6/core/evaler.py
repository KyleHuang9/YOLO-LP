...
class Evaler:
    ...
    def predict(self, model, dataloader, task):
        ...
        for i, (imgs, targets, paths, shapes) in enumerate(pbar):
            ...
            targets[:, 9:13] = xywh2xyxy(targets[:, 9:13])
            batch_targets = [torch.zeros((0, 20), device=self.device)] * imgs.shape[0]
            for target in targets:
                for j in range(9, 21, 2):
                    target[j] = target[j] * w
                    target[j + 1] = target[j + 1] * h
                batch_targets[int(target[0])] = torch.cat((batch_targets[int(target[0])], target[None, 1:]), dim=0)
            ...
            # post-process
            t3 = time_sync()
            # output[n, box + corners + conf * 8 + pred * 8]
            ...
            pred_results.append(outputs)
            total_targets.append(batch_targets)

            if i == 0:
                vis_num = min(len(imgs), 8)
                vis_outputs = outputs[:vis_num]
                vis_imgs = imgs[:vis_num]
            
        return pred_results, total_targets, vis_outputs, vis_imgs
    
    def eval(self, preds, targets, model, task):
        LOGGER.info(f'\nEvaluating speed.')
        self.eval_speed(task)
        LOGGER.info(f'\nEvaluate mAP.')
        assert len(preds) == len(targets), 'predict imgs count is not match with targets!'

        # init value
        iou_list = [0.5 + i * 0.05 for i in range(10)]
        cor_mAP_list = [0.0 for i in range(10)]
        cls_mAP_list = [0.0 for i in range(10)]
        mAP_list = [0.0 for i in range(10)]
        mAP_50 = 0.0
        mAP_75 = 0.0
        mAP_50_95 = 0.0
        true_cnt = 0
        pred_cnt = 0
        pred_cnts = [0 for i in range(10)]
        cor_right_cnt = [0 for i in range(10)]
        cls_right_cnt = [0 for i in range(10)]
        right_cnt = [0 for i in range(10)]
        recall_list = [0.0 for i in range(10)]

        for i in range(len(targets)):
            # eval one batch
            pred_batch = preds[i] # [bs, n, 26]
            target_batch = targets[i] # [bs, n, ]
            assert len(pred_batch) == len(target_batch), 'predict batch size is not match with targets'
            for j in range(len(target_batch)):
                # eval one image
                pred = pred_batch[j]
                target = target_batch[j]
                pred_box = pred[:, :4]
                target_box = target[:, 8:12]

                true_cnt += target.shape[0]
                
                if pred_box.shape[0] != 0 and target_box.shape[0] != 0:
                    iou = box_iou(pred_box, target_box)
                    iou, match = torch.max(iou, 0) # [m]
                    for k in range(len(iou)):
                        ...
                        for n in range(len(iou_list)):
                            if t_iou >= iou_list[n] and t_iou < (iou_list[n] + 0.05):
                                iou_idx = n
                                break

                        t_pred = pred[obj_idx]
                        t_target = target[k]
                        pred_cor = t_pred[4:12]
                        target_cor = t_target[12:]
                        pred_cls = t_pred[20:]
                        target_cls = t_target[:8]
                        target_box_ = t_target[8:12]
                        area = (target_box_[2] - target_box_[0]) * (target_box_[3] - target_box_[1])

                        is_cor, is_cls = False, False
                    
                        # corners (diff < 0.1a)
                        if torch.abs(pred_cor - target_cor).sum() / 8.0 < 0.1 * torch.sqrt(area).sum():
                            cor_right_cnt[iou_idx] += 1
                            is_cor = True
                        
                        # cls
                        cls_cnt = 0
                        for i in range(len(target_cls)):
                            if int(pred_cls[i].item()) == int(target_cls[i].item()):
                                cls_cnt += 1
                        if cls_cnt == 8:
                            cls_right_cnt[iou_idx] += 1
                            is_cls = True

                        # total
                        if is_cor and is_cls:
                            right_cnt[iou_idx] += 1

                    # pred iou cnt
                    iou = box_iou(pred_box, target_box)
                    iou, match = torch.max(iou, dim=0) # [n]
                    for k in range(len(iou)):
                        if iou[k] < 0.5:
                            continue
                        for n in range(len(iou_list)):
                            if iou[k] >= iou_list[n] and iou[k] < (iou_list[n] + 0.05):
                                pred_cnts[n] += 1
                                break
        ...
        # mAP.5@.95
        for i in range(10):
            cor_mAP_list[i] = cor_right_cnt[i] / pred_cnts[i] if pred_cnts[i] > 0 else 0.0
            cls_mAP_list[i] = cls_right_cnt[i] / pred_cnts[i] if pred_cnts[i] > 0 else 0.0
            mAP_list[i] = right_cnt[i] / pred_cnts[i] if pred_cnts[i] > 0 else -int(right_cnt[i] == pred_cnts[i])
            mAP_50_95 += mAP_list[i] if mAP_list[i] != -1 else 0.0
            t_50_95_cnt += 1 if mAP_list[i] != -1 else 0
            right_50 += right_cnt[i]
            pred_50 += pred_cnts[i]
            if iou_list[i] >= 0.75:
                right_75 += right_cnt[i]
                pred_75 += pred_cnts[i]
            if iou_list[i] >= 0.7:
                t_right_cnt += right_cnt[i]
        mAP_50_95 = mAP_50_95 / t_50_95_cnt if t_50_95_cnt > 0 else 0.0

        # mAP@.5 and mAP@.75 and mAP
        mAP_50 = right_50 / pred_50 if pred_50 > 0 else 0.0
        mAP_75 = right_75 / pred_75 if pred_75 > 0 else 0.0
        mAP = t_right_cnt / pred_cnt if pred_cnt > 0 else 0.0

        # recall
        recall = 0
        for i in range(10):
            for j in range(i + 1):
                recall_list[i] += right_cnt[j]
            recall_list[i] = recall_list[i] / true_cnt if true_cnt > 0 else 0.0
            recall += right_cnt[i]
        recall = recall / true_cnt
        
        return [mAP, mAP_50, mAP_75, mAP_50_95, recall, mAP_list, recall_list]
