...
from yolov6.data.show import cv2AddChineseText
...
class Trainer:
    def __init__(self, args, cfg, device):
        ...
        self.num_pro = self.data_dict['npro']
        self.num_alp = self.data_dict['nalp']
        self.num_ads = self.data_dict['nads']
        ...
        model = self.get_model(args, cfg, self.num_pro, self.num_alp, self.num_ads, device)
        if self.args.distill:
            if self.args.fuse_ab:
                LOGGER.error('ERROR in: Distill models should turn off the fuse_ab.\n')
                exit()
            self.teacher_model = self.get_teacher_model(args, cfg, self.num_pro, self.num_alp, self.num_ads, device)
        if self.args.quant:
        ...
        self.model.nc, self.model.names = self.data_dict['npro'], self.data_dict['names']
        ...
        self.loss_num = 7
        self.loss_info = ['Epoch', 'iou_loss', 'cor_loss', 'dfl_loss', 'cls_loss', 'pro_loss', 'alp_loss', 'ads_loss']
        if self.args.distill:
            self.loss_num += 1
    ...
    def eval_and_save(self):
        ...
        is_val_epoch = ((not self.args.eval_final_only or (remaining_epochs == 1)) and ((self.epoch + 1) % eval_interval == 0)) or (remaining_epochs == 1)
        if self.main_process:
            self.ema.update_attr(self.model, include=['nc', 'names', 'stride']) # update attributes for ema model
            if is_val_epoch:
                self.eval_model()
                self.ap = self.evaluate_results[0]
    ...
    def eval_model(self):
        if not hasattr(self.cfg, "eval_params"):
            results, vis_outputs, vis_imgs = eval.run(self.data_dict,
        ...
        LOGGER.info(f"Epoch: {self.epoch} | AP: {results[0]} | mAP@0.5: {results[1]} | mAP@0.50:0.95: {results[3]} | recall: {results[4]}")
        self.evaluate_results = results[:2] + results[3:5]
        # plot validation predictions
        self.plot_val_pred(vis_outputs, vis_imgs)
    ...
    def train_before_loop(self):
        ...
        self.evaluate_results = (0, 0, 0, 0) # AP, AP50, AP50_95, recall
        
        self.compute_loss = ComputeLoss(npro=self.data_dict['npro'],
                                        nalp=self.data_dict['nalp'],
                                        nads=self.data_dict['nads'],
    ...
    def get_model(self, args, cfg, npro, nalp, nads, device):
        model = build_model(cfg, npro, nalp, nads, device, fuse_ab=self.args.fuse_ab, distill_ns=self.distill_ns)
    ...
    def get_teacher_model(self, args, cfg, npro, nalp, nads, device):
        teacher_fuse_ab = False if cfg.model.head.num_layers != 3 else True
        model = build_model(cfg, npro, nalp, nads, device, fuse_ab=teacher_fuse_ab)
    ...
    def plot_val_pred(self, vis_outputs, vis_imgs, vis_conf=0.3, vis_max_box_num=5):
        # plot validation predictions
        self.vis_imgs_list = []
        for (vis_output, vis_img) in zip(vis_outputs, vis_imgs):
            vis_output_array = vis_output.cpu().numpy()     # xyxy
            ori_img = (vis_img * 255).permute(1, 2, 0).cpu().numpy().astype(np.uint8).copy()
            for idx, output in enumerate(vis_output_array):
                x_tl = int(output[0])
                y_tl = int(output[1])
                x_br = int(output[2])
                y_br = int(output[3])
                corners = output[4:12]
                score = (output[12] + output[13] + output[14] + output[15] + output[16] + output[17] + output[18] + output[19]) / 8.0
                pro_id = int(output[20])
                alp_id = int(output[21])
                ad0_id = int(output[22])
                ad1_id = int(output[23])
                ad2_id = int(output[24])
                ad3_id = int(output[25])
                ad4_id = int(output[26])
                ad5_id = int(output[27])
                # draw top n bbox
                if score < vis_conf or idx > vis_max_box_num:
                    break
                cv2.rectangle(ori_img, (x_tl, y_tl), (x_br, y_br), tuple([int(x) for x in self.color[pro_id]]), thickness=1)
                cv2.line(ori_img, (int(corners[0]), int(corners[1])), (int(corners[2]), int(corners[3])), (0, 255, 255), 1)
                cv2.line(ori_img, (int(corners[2]), int(corners[3])), (int(corners[4]), int(corners[5])), (0, 255, 255), 1)
                cv2.line(ori_img, (int(corners[4]), int(corners[5])), (int(corners[6]), int(corners[7])), (0, 255, 255), 1)
                cv2.line(ori_img, (int(corners[6]), int(corners[7])), (int(corners[0]), int(corners[1])), (0, 255, 255), 1)
                cv2.putText(ori_img, f"{pro_id}{self.data_dict['alps'][alp_id]}{self.data_dict['ads'][ad0_id]}{self.data_dict['ads'][ad1_id]}{self.data_dict['ads'][ad2_id]}{self.data_dict['ads'][ad3_id]}{self.data_dict['ads'][ad4_id]}{self.data_dict['ads'][ad5_id]}: {score:.2f}",
                        (x_tl, y_tl - 10), cv2.FONT_HERSHEY_COMPLEX, 0.5, tuple([int(x) for x in self.color[pro_id]]), thickness=1)
            self.vis_imgs_list.append(torch.from_numpy(ori_img.copy()))
